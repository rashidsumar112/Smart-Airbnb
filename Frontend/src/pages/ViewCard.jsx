...
function ViewCard() {
  ...
  const getOverlappingRange = (start, end) => {
    return bookedDates.find((period) => {
      const bookedStart = new Date(period.checkIn)
      const bookedEnd = new Date(period.checkOut)
      return new Date(start) < bookedEnd && new Date(end) > bookedStart
    })
  ...
  return (
    <div className='w-[100%] h-[100vh] bg-[white] flex items-center justify-center gap-[2px] flex-col overflow-auto relative'>
      ...
      {showBookedPopup && bookedDates.length > 0 && (
        <div className='w-[min(92%,520px)] bg-[#fff8dc] border-2 border-[#ff6b6b] rounded-xl shadow-lg p-[16px] absolute top-[180px] z-[120]'>
          <div className='flex items-center justify-between mb-[10px]'>
            <h3 className='text-[16px] font-bold text-[#d32f2f]'>Booked dates</h3>
            <button
              className='w-[28px] h-[28px] rounded-full bg-[#d32f2f] text-white'
              onClick={() => setShowBookedPopup(false)}
            >
              ×
            </button>
          </div>
          <div className='flex flex-col gap-[8px] max-h-[180px] overflow-auto'>
            {bookedDates.map((period, idx) => (
              <div key={idx} className='bg-white rounded-lg px-[10px] py-[8px] text-[14px] text-[#5c4a1f]'>
                {formatDate(period.checkIn)} - {formatDate(period.checkOut)}
              </div>
            ))}
          </div>
          <p className='mt-[10px] text-[13px] text-[#5c4a1f]'>
            You can still reserve this listing for dates outside these ranges.
          </p>
        </div>
      )}
        ...
        <div className='w-[100%] h-[100%] flex items-center justify-center flex-col gap-[20px] bg-[#ffffffcd] absolute top-[0px] z-[100] backdrop-blur-sm  p-[20px]  md:flex-row md:gap-[100px]'>
          ...
          <form className='max-w-[450px] w-[90%] h-[450px] overflow-auto bg-[#aee884] p-[20px] rounded-lg flex items-center justify-start flex-col gap-[10px]  order-[1px] border-[#dedddd] ' onSubmit={(e) => {
            ...
            if (isRangeOverlapping(checkIn, checkOut)) {
              const conflict = getOverlappingRange(checkIn, checkOut)
              const conflictMessage = conflict
                ? `This listing is already booked from ${formatDate(conflict.checkIn)} to ${formatDate(conflict.checkOut)}.`
            ...
            {bookedDates.length > 0 && (
              <div className='w-[100%] bg-[#fff8dc] border border-[#f3c46b] rounded-lg p-[10px] text-[13px] text-[#5c4a1f]'>
                <p className='font-semibold mb-[6px]'>Booked dates on this listing:</p>
                <div className='flex flex-col gap-[4px] max-h-[90px] overflow-auto'>
                  {bookedDates.map((period, idx) => (
                    <span key={idx} className='bg-white rounded px-[8px] py-[4px]'>
                      {formatDate(period.checkIn)} - {formatDate(period.checkOut)}
                    </span>
                  ))}
                </div>
                <p className='mt-[6px]'>You can still book any dates that do not overlap with the ranges above.</p>
              </div>
            )}
            ...
            <div className='w-[95%] h-[60%] border-[1px] border-[#abaaaa] rounded-lg flex justify-start items-start p-[20px] gap-[15px] flex-col '>
              ...
              <p className='w-[100%] flex justify-between items-center px-[20px]'>
                <span className='font-semibold'>
                  {`Rs_${cardDetails.rent} x ${night} Night`}
                </span>
